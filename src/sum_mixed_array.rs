// https://www.codewars.com/kata/57eaeb9578748ff92a000009/solutions/rust

use either::Either;

fn sum_mix(arr: &[Either<i32, String>]) -> i32 {
    arr.iter().fold(0, |acc, val| {
        let cloned_val = val.clone();
        if val.is_left() {
            acc + cloned_val.left().unwrap()
        } else {
            acc + cloned_val.right().unwrap().parse::<i32>().unwrap()
        }
    })
}
